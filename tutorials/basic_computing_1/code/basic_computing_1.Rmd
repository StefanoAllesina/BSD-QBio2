...
`NA` stands for Not Available. Other special values are `NaN` (Not a Number), `Inf` (Infinity), and `NULL` (variable not set). You can test for special values using `is.na(x)`, `is.infinite(x)`, etc.
...
B <- matrix(1, 3, 2) # by specifying only one value (1) it will be replicated 
B
B %*% t(B) # transpose
...
This can be problematic when your code for example expects an array and `R` turns your data into a matrix (or you expect a matrix but find a vector). To avoid this behavior, add `drop = FALSE` when subsetting:
```{r}
dim(M[,,1, drop = FALSE])
```
...
write.csv("MyFile.csv", row.names = TRUE) # Include the row names.
write.csv("MyFile.csv", col.names = FALSE) # Do not include column names.
```
