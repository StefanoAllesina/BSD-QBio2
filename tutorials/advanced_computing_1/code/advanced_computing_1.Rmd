...
pl <- ggplot(data = divvy_stations) + 
  aes(x = longitude, y = latitude, colour = factor(dpcapacity)) + 
  geom_point()
...
In many cases, we would like to produce a multi-panel graph, in which each panel shows the data for a certain combination of parameters. In `ggplot` this is easy to do using `faceting`: the command `facet_grid` is used when you want to produce a grid of panels, in which all the panels in the same row (column) have a parameter in common; `facet_wrap` is used when the different panels do not have features in common.
