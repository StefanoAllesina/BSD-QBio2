...
# =============================================
# END Copied from plot_2Dpsth.R
# =============================================

# This exercise involves estimating the probability that you will observe a spike count
# of size n over the time window starting from 0 up to time T for all T.  This is the cumulative
# spike count, which will be larger for directions near the preferred direction (peak firing rate)
# and smaller for the off-preferred directions
...
for (n in 1:nDirs){
  counts[n, ] <- colSums(mydata[Time, n, ])  #more properly colSums(mydata[Time,n,1:nReps[n]])
                                              # but for array positions > nReps[n] you are just 
                                              # summing over 0s
}
...
for (n in 1:nDirs){
  ...
  #hh is a structure.  We want to save parts of the structure as different
  # variable names.  In R, elements of the structure (fields) are indexed 
  # with a dollar sign, $
  
  # Get the midpoints for each bin, which we will use for plotting
  mids <- hh$mids    
  
  # The probability of the count given the direction is built from the field
  # "counts" in hh normalized
